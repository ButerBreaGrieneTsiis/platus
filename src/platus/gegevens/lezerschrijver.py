...
def decoder(dictionary, **kwargs):
    ...
    mapping     =   {
                        ...
                        "bedrijf": (frozenset(("naam", "iban", "rekeningnummer", "giro", "synoniemen", "uitsluiten", "categorie")), platus.Bedrijf.van_json),
                        }
    
    # for cls, cls_tuple in mapping.items():
    #     if "class" in kwargs.keys():
    #         if cls == kwargs["class"]:
    #             if cls_tuple[0].issuperset(dictionary.keys()):
    #                 return cls_tuple[1](**dictionary)
