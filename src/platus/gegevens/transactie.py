...
class Transactie:
        ...
        while True:
            ...
            if opdracht.get("opdracht") == "":
                ...
                for iveld, (veld, waarde) in enumerate(self.__dict__.items()):
                    ...
                    elif isinstance(waarde, dt.datetime):
                        print(f"\t{iveld:<6}{veld:<35}{dt.datetime.strftime(waarde, "%A %d %B %Y, %H:%M")}")
                    else:
    ...
    def bewerken(
        ...
        if veld == "derde_uuid":
            ...
            while True:
                ...
                print(f"\nkies een bestaande derde met \"zoek <zoekterm>\" of een nieuwe met \"nieuw\"")
                ...
                if opdracht.get("opdracht") == "nieuw":
                    ...
                    if derde_type == "bedrijf":
                        if "naam" in self.tijdelijk.keys():
                            print(f"toevoegen synoniemn \"{self.tijdelijk["naam"]}\" aan bedrijf \"{naam}\"?")
                            synoniemen = [self.tijdelijk["naam"].casefold()] if invoer_kiezen("keuze", {"ja": True, "nee": False}) else []
                            if "derde_iban" in self.tijdelijk.keys():
                                bedrijf     =   Bedrijf(naam, iban = [self.tijdelijk.get("derde_iban")], synoniemen = synoniemen)
                            else:
                                bedrijf     =   Bedrijf(naam, synoniemen = synoniemen)
                        else:
                            if "derde_iban" in self.tijdelijk.keys():
                    ...
                    if derde_type == "bedrijf":
                        ...
                        if "naam" in self.tijdelijk.keys():
                            print(f"toevoegen synoniemn \"{self.tijdelijk.get("naam")}\" aan bedrijf \"{bedrijven[uuid].naam}\"?")
    ...
    def derde(
        ...
        if self.derde_uuid is None:
            return bedrijven[self.config["derde_onbekend"]]
        else:
